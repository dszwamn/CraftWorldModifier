...
- 本项目优先按 Steam 正式版、当前已测试的游戏版本进行定位；使用非 Steam 版本、旧版本、测试版、重新打包版或不同语言/模组组合时，部分地址和对象结构可能不同，功能可能失效，严重时有导致游戏崩溃或存档异常的风险。
- 使用前请确认游戏版本与修改器支持范围；不要把旧版本的定位结果直接套用到更新后的游戏，也不要在版本不明时强行执行写入。
